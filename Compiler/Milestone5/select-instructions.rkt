...
(define (loc? t)
  (or (or (aloc? t) (fvar? t)) (or (symbol? (isRegister? t)) (symbol? (isRegister? t)))))

(define (triv? t)
  (or (integer? t) (or (loc? t) (label? t))))
...
(define (select-func f)
  (match f
    [`(define ,l ,t) `(define ,l () ,(select-tail t))]
    [_ #f]))
...
(define (select-instructions p)
  (match p
    [`(module ,f ... ,t) `(module () ,@(map select-func f) ,(select-tail t))]
    [_ #f]))
...
;select-instructions
  ...
  (check-select (select-instructions
                 '(module (define L.test.1 (begin  (set! x.2 fv0) (set! x.3 fv1) (set! x.1 ca0) (begin (set! y.4 (+ x.1 x.2)) (+ x.3 y.4))))
                ...
                '(module ()
                   (define L.test.1
                     ...
                     (begin
                       ...
                       (begin
                         (set! y.4 (+ x.1 x.2))
                         (begin (set! x.3 (+ x.3 y.4)) (halt x.3)))))
                   (begin
                     (set! fv0 2)
                     (set! fv1 3)
                     (set! ca0 1)
