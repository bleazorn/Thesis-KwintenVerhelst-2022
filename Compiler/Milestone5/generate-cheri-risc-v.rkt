...
(define (generate-set s)
  (match s 
    [`(set! (,r - ,n) ,b) #:when (isCapability? r) (string-append (indent-instr (format "CIncOffset cs11, ~a, ~a" r n)) (indent-instr (format "sw.cap ~a, 0(cs11)" b)))]
    [`(set! ,a (,r - ,n)) #:when (isCapability? r) (string-append (indent-instr (format "CIncOffset cs11, ~a, ~a" r n)) (indent-instr (format "lw.cap ~a, 0(cs11)" a)))]
    ...
    [`(set! ,a ,b) #:when (label? b) (indent-instr (format "cllc ~a, ~a" a b))]
    [`(set! ,a ,b) (indent-instr (format "addi ~a, ~a, 0" a b))]
    [`(with-label ,l ,a) (format "~a:\n~a" l (generate-set a))]
    [`(jump ,l) #:when (isCapability? l) (indent-instr (format "cjr ~a" l))]
    [`(jump ,l) (string-append (indent-instr (format "cllc cs11, ~a" l)) (indent-instr (format "cjr cs11")))]
...
(define (generate-cheri-risc-v p)
    (match p
    [`(,info (begin ,s ...)) (cons info (foldl (lambda (s string) (format "~a~a" string (generate-set s))) "" s))]
    [_ #f]))
...
;generate-set
  ...
  (check-equal? (generate-set '(set! a0 (cfp - 0))) "    CIncOffset cs11, cfp, 0\n    lw.cap a0, 0(cs11)\n" "generate-set: succes-06: load from memory")
  (check-equal? (generate-set '(set! (cfp - 0) a0)) "    CIncOffset cs11, cfp, 0\n    sw.cap a0, 0(cs11)\n" "generate-set: succes-07: store in memory")
  ...
  (check-equal? (generate-set '(jump foo))  "    cllc cs11, foo\n    cjr cs11\n" "generate-set: succes-10: compare")
...
;generate-cheri-risc-v
  (check-equal? (generate-cheri-risc-v '(0 (begin (set! a0 (+ t0 450))))) '(0 . "    addi a0, t0, 450\n") "generate-cheri-risc-v: succes-1: een enkele instructie")
