...
;locs: list? '(name? ...)
(define (check-let l locs)
  (match l
    [`(,x ,v) `(,x ,(check-value v (cons x locs)))]
...
;p: pred?
;locs: list? '(name? ...)
(define (check-predi p locs)
  (match p
    [`(let (,l ...) ,pred) `(let ,(map (lambda (le) (check-let le locs)) l) ,(check-predi pred (append locs (map car l))))]
    [`(if ,p1 ,p2 ,p3)     `(if ,(check-predi p1 locs) ,(check-predi p2 locs) ,(check-predi p3 locs))]
    [`(,relop ,t1 ,t2)     `(,(check-relop relop) ,(check-triv t1 locs) ,(check-triv t2 locs))]
    ['(true)               '(true)]
    ['(false)              '(false)]
    [`(not ,pred)          `(not ,(check-predi pred locs))]
...
;v: value?
;locs: list? '(name? ...)
(define (check-value v locs)
  (match v
    [`(let (,l ...) ,val) `(let ,(map (lambda (le) (check-let le locs)) l) ,(check-value val (append locs (map car l))))]
    [`(if ,p ,v1 ,v2)     `(if ,(check-predi p locs) ,(check-value v1 locs) ,(check-value v2 locs))]
...
(define (check-tail t locs funcs)
  (match t
    [`(let (,l ...) ,tail) `(let ,(map (lambda (le) (check-let le locs)) l) ,(check-tail tail (append locs (map car l)) funcs))]
    [`(if ,p ,t1 ,t2)      `(if ,(check-predi p locs) ,(check-tail t1 locs funcs) ,(check-tail t2 locs funcs))]
    [`(call ,x ,trivs ...) `(call ,(check-name x funcs) ,@(map (lambda (x) (check-triv x locs)) trivs))]
    [v                      (check-value v locs)]))
