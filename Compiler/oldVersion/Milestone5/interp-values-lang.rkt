...
(define (interp-pred p locs)
  (match p
    ...
    [`(if ,p1 ,p2 ,p3) (if (interp-pred p1 locs)
                           (interp-pred p2 locs)
                           (interp-pred p3 locs))]
...
(define (interp-value v locs)
  (match v
    ...
    [`(if ,p ,v1 ,v2) (if (interp-pred p locs)
                          (interp-value v1 locs)
                          (interp-value v2 locs))]
...
(define (interp-tail t locs funcs)
  ...
  (if (< step maxsteps)
      (match t
        ...
        [`(if ,p ,t1 ,t2) (if (interp-pred p locs)
                              (interp-tail t1 locs funcs)
                              (interp-tail t2 locs funcs))]
        [`(call ,x ,trivs ...) (interp-func x trivs locs funcs)]
