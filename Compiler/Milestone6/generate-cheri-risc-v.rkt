...
(define (generate-set s)
  (match s
    [`(set! (,r - ,n) ,b) (string-append (indent-instr (format "CIncOffset ~a, ~a, ~a" (current-stack-register) r (- 0 n)))
                                         (indent-instr (format "sw.cap ~a, 0(~a)" b (current-stack-register))))]                ;store
    [`(set! ,a (,r - ,n))(string-append (indent-instr (format "CIncOffset ~a, ~a, ~a" (current-stack-register) r (- 0 n)))
                                        (indent-instr (format "lw.cap ~a, 0(~a)" a (current-stack-register))))]                 ;load
