...
(define (generate-jump j)
  (match j
    ...
    [`(jump (,r - ,n)) (string-append (indent-instr (format "CIncOffsetImm ~a, ~a, ~a" (current-stack-register) (makeCap r) (- 0 n)))                     ;jump addr
                                      (indent-instr (format "lc.cap ct5, 0(~a)" (current-stack-register)))
...
(define (split)
  (string-append 
   (indent-instr (format "cmove cfp, csp"))
   (indent-instr (format "CIncOffset cfp, cfp, s5"))
   (indent-instr (format "CSetBounds cfp, cfp, s4"))
   (indent-instr (format "CIncOffset cfp, cfp, s4"))
   
   (indent-instr (format "CGetBase t0, csp"))
   (indent-instr (format "CGetLen t1, csp"))
   (indent-instr (format "sub t1, x0, t1"))
   
   (indent-instr (format "CIncOffset csp, csp, t1"))
   
   (indent-instr (format "sub t1, x0, t1"))
   (indent-instr (format "Add t1, t1, s5"))
   (indent-instr (format "CSetBounds csp, csp, t1"))
   (indent-instr (format "CIncOffset csp, csp, t1")))
  )
...
(define (splice)
  (string-append 
   (indent-instr (format "cgetbase t0, csp"))
   (indent-instr (format "cgetlen t1, csp"))
   (indent-instr (format "add t1, t0, t1"))
   (indent-instr (format "cgetbase t2, cfp"))
   (indent-instr (format "BNE t1, t2, error"))
   
   (indent-instr (format "cgetbase t2, ct6"))
   (indent-instr (format "BNE t0, t2, error"))
   
   (indent-instr (format "cMove csp, ct6")))
  )
...
(define (generate-sets s)
  (match s 
    ...
    [`(seal ,r ... ,s) (generate-seal s)]                          ;seal
    [`(unseal ,r ... ,s) (generate-unseal s)]                      ;unseal
    [`(split ,a ,b ,c ,d) (split)]                                 ;split
    [`(splice ,a ,b ,c ,d) (splice)]                               ;splice
