...
(provide wrap-cheri-risc-v-run-time-cheri-linkage-seal)
...
;p: string?
(define (wrap-cheri-risc-v-run-time-cheri-linkage-seal p)
  (pretty-log p)
  (match p
    [`(begin ,i ,s) #:when (string? s) (string-append "main:\n"
                                                      ...
                                                      (indent-instr (format "cllc cs11, main.size"))
                                                      ...
                                                      (indent-instr (format "cmove csp, cs11"))
                                                      (indent-instr (format "CSetAddr cs11, cs11, t4"))
                                                      (indent-instr (format "CSeal cra, cra, cs11"))
                                                      (indent-instr (format "CSeal csp, csp, cs11"))
                                                      "//PCC init\n"
...
(define (intitialize-framesizes framesizes)
  (if (null? framesizes)
      ...
      (let ([name (car (car framesizes))]
        ...
        (string-append
         (indent-instr (format "~a.size: .int ~a" name (* size (framesize))))
