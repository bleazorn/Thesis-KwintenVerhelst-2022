...
(define (assign-info i)
  (let ([calls  (getInfo i getCallUndead)]
    ...
    (let* ([nonLocConf (filter (lambda (c) (member (car c) calls)) confs)]
           [newAss (assign-call-undead calls confs (remove-duplicates (append (filter fvar? (map car confs)) allFva)) nonLocConf)]);(filter (lambda (a) (fvar? (second a))) ass) nonLocConf)])
