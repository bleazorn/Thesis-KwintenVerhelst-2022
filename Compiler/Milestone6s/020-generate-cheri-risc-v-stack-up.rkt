...
(require "common/register.rkt"
         ...
         "common/fvar.rkt"
         "setup.rkt")
(provide generate-cheri-risc-v-stack-up)
...
(define (generate-set s)
  (match s
    [`(set! (,r - ,n) ,b) #:when (isCapability? b) (string-append (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (+ 0 n)))
                                                                  (indent-instr (format "sc.cap ~a, 0(~a)" b r))
                                                                  (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (- 0 n))))]                ;store
    [`(set! (,r - ,n) ,b) #:when (isNonCapRegister? b) (string-append (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (+ 0 n)))
                                                                      (indent-instr (format "sw.cap ~a, 0(~a)" b r))
                                                                      (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (- 0 n))))]
    [`(set! ,a (,r - ,n)) #:when (isCapability? a) (string-append (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (+ 0 n)))
                                                                  (indent-instr (format "lc.cap ~a, 0(~a)" a r))
                                                                  (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (- 0 n))))]                 ;load
    [`(set! ,a (,r - ,n)) #:when (isNonCapRegister? a) (string-append (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (+ 0 n)))
                                                                      (indent-instr (format "lw.cap ~a, 0(~a)" a r))
                                                                      (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (- 0 n))))]
...
(define (generate-jump j)
  (match j
    ...
    [`(jump (,r - ,n)) (string-append (indent-instr (format "CIncOffsetImm ~a, ~a, ~a" (makeCap r) (makeCap r) (+ 0 n)))                                  ;jump addr
                                      (indent-instr (format "lc.cap ct5, 0(~a)" (makeCap r)))
                                      (indent-instr (format "CIncOffsetImm ~a, ~a, ~a" (makeCap r) (makeCap r) (- 0 n)))
                                      (indent-instr (format "cjr ct5")))]
...
(define (generate-jump-if j)
  (match j
    ...
    [`(jump-if ,l (> ,b ,c))  (indent-instr (format "blt ~a, ~a, ~a" c b l))]
    [`(jump-if ,l (<= ,b ,c)) (indent-instr (format "bge ~a, ~a, ~a" c b l))]
...
(define (generate-setLinear s)
  (match s
    [`(setLinear! (,r - ,n) ,b) (string-append (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (+ 0 n)))
                                               (indent-instr (format "sl.cap ~a, 0(~a)" b r))
                                               (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (- 0 n))))]
    [`(setLinear! ,a (,r - ,n)) (string-append (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (+ 0 n)))
                                               (indent-instr (format "ll.cap ~a, 0(~a)" a r))
                                               (indent-instr (format "CIncOffset ~a, ~a, ~a" r r (- 0 n))))]
...
(define (generate-sets s)
  (match s 
    ...
    [`(with-label ,l ,a) (string-append (format "~a:\n" l)
                                        (indent-instr (format "addi s6, s6, ~a\n~a" (oneUpLabel) (generate-sets a))))]   ;set label 
...
; p: any?
(define (generate-cheri-risc-v-stack-up p)
    (match p
...
;generate-cheri-risc-v-stack-up
  (check-equal? (generate-cheri-risc-v-stack-up '(begin (set! a0 (+ t0 450)))) "    addi a0, t0, 450\n" "generate-cheri-risc-v-stack-up: succes-1: een enkele instructie")
  (check-equal? (generate-cheri-risc-v-stack-up
                 '(begin
                    (set! a0 50)
                    (set! t0 a0)
                    (set! t0 (+ t0 t0))
                    ...
                    (set! a1 2000)))
                "    addi a0, zero, 50\n    addi t0, a0, 0\n    add t0, t0, t0\n    addi sp, t0, 0\n    mul sp, sp, sp\n    addi a1, zero, 2000\n"
                "generate-cheri-risc-v-stack-up: succes-2: meerdere instructies")
  
;failure 
  (check-equal? (generate-cheri-risc-v-stack-up '(set! a0 (+ a0 450))) #f "generate-cheri-risc-v-stack-up: failure-1: vergeet initialisatie")
 #|
  (check-equal? (generate-cheri-risc-v-stack-up
                 '(begin
                    (set! a0 50)
                    ...
                    (set! a1 2000)))
                error
                "generate-cheri-risc-v-stack-up: failure-2: meerdere instructies met 1 slechte instructie")
|#
  )
