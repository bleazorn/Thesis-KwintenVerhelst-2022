...
(define (cap-binop e)
  (match e
    [`(set! ,a (+ ,b ,c)) #:when (and (isCapability? a) (isCapability? b) (register? c))
                               `(set! ,(makeCap a) (+ ,(makeCap b) ,(makeReg c)))]
    [`(set! ,a (+ ,b ,c)) #:when (and (or (and (isCapability? a) (isNonCapRegister? b)) (and (isNonCapRegister? a) (isCapability? b))) (integer? c))
                               `(set! ,(makeReg a) (+ ,(makeReg b) ,c))]
    [`(set! ,a (,binop ,b ,c)) #:when (and (register? a) (register? b) (register? c))
                               `(set! ,(makeReg a) (,binop ,(makeReg b) ,(makeReg c)))]
    [_ e]))
...
(define (cap-set e)
  (match e
    [`(set! (,a - ,n) ,b)                                                 `(set! (,(makeCap a) - ,n) ,b)]
    [`(set! ,a (,b - ,n))                                                 `(set! ,a (,(makeCap b) - ,n))]
...
(define (cap-jump e)
  (match e
    [`(jump ,r) #:when (register? r) `(jump ,(makeCap r))]
    [`(jump (,r - ,n)) #:when (register? r) `(jump (,(makeCap r) - ,n))]
    [_ e]))    
...
(define/contract (cap-mode-on p) (-> paren-cheri-risc-v? paren-cheri-risc-v?)
  (match p
