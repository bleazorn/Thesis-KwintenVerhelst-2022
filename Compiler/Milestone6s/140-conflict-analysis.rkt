...
;
;(conflict-info info tail)->info?
;info: info?
;tail: tail?
(define (conflict-info info tail)
  (addInfo info (setConflicts (conflict-tail tail (getInfo info getUndead-out) (conflict-locals (getInfo info getLocals))))))
;
;(conflict-func f)->'(define label? info? tail?)   info?:'(locals conflicts)
;f: '(define label? info? tail?)
(define (conflict-func f)
  (match f
    [`(define ,l ,i ,t) `(define ,l ,(conflict-info i t) ,t)]
    [_ #f]))
