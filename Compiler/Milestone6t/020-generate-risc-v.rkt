...
(define (generate-set s)
  (match s
    [`(set! (,r ,binop ,n) ,b) #:when (register? r) (indent-instr (format "sw ~a, ~a~a(~a)" b binop n r))]      ;store
    [`(set! ,a (,r ,binop ,n)) #:when (register? r) (indent-instr (format "lw ~a, ~a~a(~a)" a binop n r))]      ;load
...
(define (generate-jump j)
  (match j
    ...
    [`(jump (,r ,binop ,n)) (string-append (indent-instr (format "lwu s9, ~a~a(~a)" binop n r)) ;jump addr
