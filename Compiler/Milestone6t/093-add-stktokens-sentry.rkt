...
(define (add-func f)
  (match f
    ...
    [_ (error (format "add-stktokens-sentry:  Failed match.\n No valid function: ~a" f))]))
...
(module+ test
  ...
  (check-equal? (add-tail '(jump-call L.swap.1))
                '(begin (set! cs1 pcc) (set! t5 2147491840) (set-addr! cs1 t5) (jump-call L.swap.1))
                "add-tail: succes-01: jump call")
  (check-equal? (add-tail '(jump-return cra))
                '(begin (set! cs1 pcc) (set! t5 2147491840) (set-addr! cs1 t5) (jump-return cra))
                "add-tail: succes-02:jump return")
  (check-equal? (add-tail '(begin (set! cra fv0) (jump-return cra)))
                '(begin (set! cs1 pcc) (set! t5 2147491840) (set-addr! cs1 t5) (begin (set! cra fv0) (jump-return cra)))
                "add-tail: succes-03: begin")
  (check-equal? (add-tail '(if (= t0 t1) (jump-return cra) (jump-call L.swap.1)))
                '(begin (set! cs1 pcc) (set! t5 2147491840) (set-addr! cs1 t5) (if (= t0 t1) (jump-return cra) (jump-call L.swap.1)))
                "add-tail: succes-04: if")
  (check-equal? (add-tail '(invoke cra cfp))
                '(begin (set! cs1 pcc) (set! t5 2147491840) (set-addr! cs1 t5) (invoke cra cfp))
